...
export const useJobHuntFirestore = (userId: string) => {
  ...
  // Resume – stored at resumes/{userId} so one document per user
  const saveResume = async (resumeData: object) => {
  ...
  const loadResume = async (): Promise<object | null> => {
    const ref = doc(db, 'resumes', userId)
    const snap = await getDoc(ref)
    if (snap.exists()) {
      return (snap.data() as any).data ?? null
    }
    return null
  }
