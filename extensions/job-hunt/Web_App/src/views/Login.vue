  ...
  <div class="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center p-4">
    <div class="w-full max-w-md bg-white rounded-lg shadow-xl p-8">
      ...
      <div v-if="error" class="mb-6 bg-red-50 border border-red-200 rounded-lg p-4">
        <template v-if="error === 'AUTH_DOMAIN_BLOCKED'">
          <p class="text-red-700 text-sm font-semibold mb-1">Authentication not configured for this domain.</p>
          <p class="text-red-600 text-xs">
            The Firebase project needs to authorize <strong>app.cpfcoaching.us</strong>.
            Please contact the site administrator.
          </p>
        </template>
        <p v-else class="text-red-700 text-sm">{{ error }}</p>
      </div>
