		...
		try {
			resp = await config.fetch(url, {
				...
				headers: {
					...
					"User-Agent": config.userAgent || `anthropic-sdk-typescript/0.93.0 oidcFederationProvider`
				},
				body: JSON.stringify(body)
			});
		} catch (err) {
		...
		try {
			resp = await config.fetch(url, {
				...
				headers: {
					...
					"User-Agent": config.userAgent || `anthropic-sdk-typescript/0.93.0 userOAuthProvider`
				},
				body: JSON.stringify(body)
			});
		} catch (err) {
...
async function defaultCredentials(options, profile) {
	...
	return resolveCredentialsFromConfig(config.authentication.credentials_path ? config : {
		...config,
		authentication: {
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/resources/beta/environments.mjs
