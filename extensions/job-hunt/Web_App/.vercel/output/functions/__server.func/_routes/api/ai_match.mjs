import { t as Anthropic } from "../../_libs/anthropic-ai__sdk.mjs";
//#region api/ai-match.ts
function scoreJobLocally(job, candidateSkills) {
	if (!candidateSkills.length) return 75;
...
async function handler(req, res) {
	...
	if (openRouterKey && jobs.length > 0) try {
...
Only return the JSON.`;
		...
		if (response.ok) {
			...
			if (content) {
				...
				if (parsed.matchedJobs) return res.status(200).json(parsed);
			}
		}
	} catch (err) {
	...
	if (anthropicKey && jobs.length > 0) try {
...
Only return the JSON.`;
		...
		if (content.type === "text") {
			...
			if (data.matchedJobs) return res.status(200).json(data);
		}
	} catch (err) {
	...
	if (aceUrl) try {
		...
		if (aceRes.ok) return res.status(200).json(await aceRes.json());
	} catch (err) {
		console.error("ACE agent fallback failed:", err);
	}
	const skills = Array.isArray(candidateSkills) ? candidateSkills : [];
	const matchedJobs = (Array.isArray(jobs) ? jobs : []).map((job) => ({
		...job,
		matchScore: scoreJobLocally(job, skills),
		matchReason: skills.length ? `Matched on: ${skills.filter((s) => (job.position ?? "").toLowerCase().includes(s.toLowerCase())).join(", ") || "general fit"}` : "Scored based on role relevance"
